...
function Install-Desktop {
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # casualty the only thing missing is node_modules\electron\dist.
            # Repopulate it via electron's own downloader (canonical source first,
            # then the public mirror when the user hasn't pinned one) and carry on
            # to the build; the mirror self-heal after `pack` below is otherwise
            # unreachable on a blocked network because this install step throws first.
            $depsRepaired = $false
            ...
            if ($depsRepaired) {
                Write-Warn "Dependency install failed on the Electron binary download; repopulated node_modules\electron\dist via a mirror and continuing."
            } else {
